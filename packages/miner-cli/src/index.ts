...
import { loadConfig, validateConfig, isFirstRun, type MinerConfig } from './config.js'
...
program
  ...
  .action(async () => {
    ...
    // Auth method choice
    console.log(chalk.bold('Step 1: Authentication Method'))
    console.log(chalk.gray('  A) Solana Wallet  (WALLET_PRIVATE_KEY) — requires a Solana wallet'))
    console.log(chalk.gray('  B) $SHELL API Key (SHELL_API_KEY)       — register at openshell.cc ✓ recommended'))
    console.log()
    const authChoice = (await ask(chalk.white('  Choose A or B [B]: '))).trim().toUpperCase() || 'B'
    ...
    if (authChoice === 'A') {
      console.log()
      console.log(chalk.gray('  Generate a wallet: solana-keygen new --outfile miner-wallet.json'))
      walletKey = (await ask(chalk.white('  WALLET_PRIVATE_KEY (base58): '))).trim()
    }
    else {
      console.log()
      console.log(chalk.gray('  Register at https://openshell.cc, go to Dashboard → Agent Registration → copy sk-shell-xxx'))
      shellApiKey = (await ask(chalk.white('  SHELL_API_KEY (sk-shell-xxx): '))).trim()
    }

    // LLM setup
    console.log()
    console.log(chalk.bold('Step 2: LLM Provider'))
    console.log(chalk.gray('  1) Anthropic (claude-haiku-4-5)   → console.anthropic.com'))
    console.log(chalk.gray('  2) OpenAI    (gpt-4o-mini)        → platform.openai.com'))
    console.log(chalk.gray('  3) DeepSeek  (deepseek-chat)      → platform.deepseek.com  💰 cheapest'))
    console.log()
    const llmChoice = (await ask(chalk.white('  Choose 1/2/3 [1]: '))).trim() || '1'
    const providerMap: Record<string, string> = { '1': 'anthropic', '2': 'openai', '3': 'deepseek' }
    const llmProvider = providerMap[llmChoice] || 'anthropic'

    const llmApiKey = (await ask(chalk.white('  LLM_API_KEY: '))).trim()
    ...
    const envContent = [
      ...
      walletKey ? `WALLET_PRIVATE_KEY=${walletKey}` : '# WALLET_PRIVATE_KEY=',
...
program
  ...
  .action(async (opts) => {
    ...
    if (isFirstRun(config)) {
      ...
      console.log()
      console.log(chalk.white('  Run the setup wizard to get started:'))
      console.log(chalk.cyan('    npx @openshell-cc/miner-cli setup'))
      console.log()
      console.log(chalk.gray('  Or manually create a .env file with:'))
      console.log(chalk.gray('    SHELL_API_KEY=sk-shell-xxx  ← from https://openshell.cc/dashboard'))
      console.log(chalk.gray('    LLM_API_KEY=sk-ant-...      ← from console.anthropic.com'))
...
async function handleSandboxVerified(
  ...
  genSpinner.succeed(`Payload generated (${response.tokensUsed.input + response.tokensUsed.output} tokens)`)
