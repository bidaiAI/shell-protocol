...
import { loadConfig, validateConfig, isFirstRun, isPlatformManaged, type MinerConfig } from './config.js'
...
program
  ...
  .action(async () => {
    ...
    // Step 2: LLM (optional)
    console.log()
    console.log(chalk.bold('Step 2: LLM API Key（可选）'))
    console.log(chalk.cyan('  不填也可以挖矿') + chalk.gray(' — 平台 AI 自动处理，直接按 Enter 跳过'))
    console.log(chalk.gray('  填入后切换为本地模式，速度更快、任务更多：'))
    ...
    if (llmApiKey) {
      if (llmApiKey.startsWith('sk-ant-') || llmApiKey.startsWith('sk-')) llmProvider = 'anthropic'
      else if (llmApiKey.startsWith('sk-proj-')) llmProvider = 'openai'
      else {
        console.log()
    ...
    const modeComment = llmApiKey ? 'auto' : 'sandbox_only  # 平台 AI 模式（无需自备 LLM）'
    const envContent = [
      ...
      llmApiKey ? `LLM_API_KEY=${llmApiKey}` : '# LLM_API_KEY=   # 留空则使用平台 AI 自动挖矿',
      '',
      '# Mining',
      `EXECUTION_MODE=${modeComment}`,
...
program
  ...
  .action(async (opts) => {
    ...
    if (isFirstRun(config)) {
      ...
      console.log(chalk.gray('    # LLM_API_KEY 可选 — 不填则平台 AI 自动处理'))
    ...
    const modeLabel = isPlatformManaged(config)
      ? chalk.cyan('sandbox_only') + chalk.gray(' (平台 AI 模式 — 无需自备 LLM)')
    ...
    if (isPlatformManaged(config)) {
      console.log(chalk.gray('  ') + chalk.green('✓') + chalk.gray(' 平台 AI 负责生成攻击载荷，你无需提供 LLM API Key'))
    }
    console.log()
