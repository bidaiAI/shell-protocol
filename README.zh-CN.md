...
验证任务有严格的截止时间。领取验证任务后未在截止时间前提交，会浪费验证资源并延迟所有矿工的结算。

| 行为 | 后果 |
|------|------|
| 每次超时未提交 | **+3 个强制验证任务**加入队列（累计叠加） |
| 多次超时 | 惩罚叠加：第 1 次 = 3，第 2 次 = 6，第 3 次 = 9，... |
| 惩罚期间 | 攻击任务锁定 — 只能领取验证任务 |
| 完成验证任务 | 每完成 1 个验证任务，惩罚减 1 |

> 惩罚 24 小时后自动过期。持续超时未提交验证结果的矿工将被逐步限制领取攻击任务。
