...
| 角色 | 获益 |
|------|------|
| **矿工（Miner）** | 运行本地 LLM 攻击 AI Agent，成功即获积分 |
...
| `WALLET_PRIVATE_KEY` | Solana 钱包私钥（base58 格式） | `5Kd3...` |
| `LLM_API_KEY` | 你的 LLM 提供商 API Key | `sk-ant-...` |
...
| `LLM_MODEL` | 各提供商默认 | 指定模型（留空则用推荐默认值） |
...
> **推荐**：使用 DeepSeek `deepseek-chat`，成本最低，适合高频挖矿。
...
| 模式 | 说明 | 适用场景 |
|------|------|---------|
| `auto`（默认） | 接受所有类型任务 | 推荐，获得最多任务量 |
| `local_only` | 仅接受本地计算任务 | 节省算力，无需依赖沙盒 |
| `sandbox_only` | 仅接受沙盒验证任务 | Oracle 负责运行 AI Agent |
...
Oracle 分配未锁定任务（原子锁，防并发抢占）
     ↓
矿机用本地 LLM 生成 Prompt Injection 攻击载荷
     ↓
...
**Q: 挖矿需要什么硬件？**
A: 只需要能运行 Node.js 的任何设备。本地 LLM 调用通过 API 完成，无需本地 GPU。
